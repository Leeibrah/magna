...
$sqlmpesa = "select * from cart inner join cart_total on cart.session_id=cart_total.session_id where order_id='$ordid'";
$rempesa = mysql_query($sqlmpesa);
$rowmpesa = mysql_fetch_array($rempesa);
...
if(mysql_num_rows($recardst)>0)
{
	...
	$message .= "<div  align='center'><a href='http://www.vitumob.com/beta'><img id='logo' src='http://www.vitumob.com/images/logo.png' width='220'></a></div>
	<div  style='background:url(http://www.vitumob.com/images/tag_without.png) no-repeat; background-position:78% top'>
<div id='main' class=''>
...
<p>Thank you for your order with VituMob. You have selected to pay with M-Pesa; to complete your payment, follow the instructions below. We'll let you know when we receive your payment. To check the status of your order at any time, click this link: <a href='http://www.vitumob.com/orders/order_detail.php?email=<?php echo $useremail?>&id=<?php echo $ordid?>&vieworder=View+Order'>http://www.vitumob.com/orders/order_detail.php?email=<?php echo $useremail?>&id=<?php echo $ordid?>&vieworder=View+Order</a>.</p>



<p style='margin-bottom: 0'>To pay with M-Pesa:</p>
...
<ol style='margin-top: 0'>
    ...
    <li>Enter the VituMob Business Number: <strong>997200</strong>
        <li>Enter your order amount: <strong><?php echo str_replace(",","",$rowmpesa['total_ksh'])?></strong>
    <li>Enter your order number as the account number:<strong><?php echo $_REQUEST['orderid']?></strong>
...
<p>
    <div>Name: <?php echo $roworderdata['name']?></div>
    <div>Phone: <?php echo $roworderdata['phone']?></div>
    <div>Email: <?php echo $roworderdata['email']?></div>
    <div>Neighbourhood: <?php echo $roworderdata['neighbourhood']?></div>
    <div>City: <?php echo $roworderdata['city']?></div>
</p>
...
$i=0;
$sqlord = "select link,host_name from cart inner join orders on cart.order_id=orders.id where order_id='".$_REQUEST['orderid']."' and status='1' group by host_name order by host_name asc";
 $reord = mysql_query($sqlord) or die(mysql_error());
